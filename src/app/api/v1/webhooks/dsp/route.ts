...
/**
 * POST /api/v1/webhooks/dsp — sandbox DSP royalty callbacks
 * (`royalty.report` | `royalty.adjusted` | `royalty.reversed`).
 */
...
export async function POST(request: NextRequest) {
  ...
  const verdict = checkRateLimit(
    `dsp-webhook:${clientIdentity(request)}`,
    DON_API_RATE_LIMIT,
  );
  if (!verdict.ok) {
    return jsonError(
  ...
  try {
    ...
    console.error("Failed DSP webhook ingest:", error);
    return jsonError(500, "store_failure", "Failed to ingest DSP webhook.");
  }
}

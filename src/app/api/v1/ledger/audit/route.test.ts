...
describe("GET /api/v1/ledger/audit", () => {
  it("returns a balanced empty sheet", async () => {
    const response = await GET(
      new NextRequest("http://localhost:3000/api/v1/ledger/audit"),
    );
    expect(response.status).toBe(200);
    const body = await response.json();
    expect(body.double_entry.balanced).toBe(true);
    expect(body.books_reconcile).toBe(true);
  });
